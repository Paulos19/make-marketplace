...
export default async function ProductsPage({ searchParams }: { searchParams: Promise<{ [key: string]: string | string[] | undefined }> }) {
  ...
  const whereClause: Prisma.ProductWhereInput = {
    isSold: false,
    isReserved: false,
    isService: false, // Só produtos físicos
  };
  ...
  const [boostedProducts, categories, sellers, filteredProducts, totalProductsCount] = await Promise.all([
    ...
    prisma.category.findMany({
      where: { products: { some: { isService: false } } },
    }),
    prisma.user.findMany({
      where: {
        role: 'SELLER',
        showInSellersPage: true,
        products: { some: { isService: false, isSold: false } }
      },
      include: { 
        products: { 
          where: { isService: false, isSold: false },
          include: { user: true, category: true },
          take: 5
        } 
      },
        ...
        <div className="flex flex-col mt-12">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
                <div>
                    ...
                    <p className="text-muted-foreground text-sm mt-1">Encontrámos {totalProductsCount} produtos para você</p>
                </div>
            ...
            <div className="w-full">
                {filteredProducts.length > 0 ? (
                    ...
                    <div className="flex flex-col items-center justify-center text-center py-24 bg-slate-50 dark:bg-slate-900/50 rounded-[2rem] border border-dashed border-border w-full">
                        <PackageOpen className="h-16 w-16 text-muted-foreground/50 mb-6" />
                        <h2 className="text-2xl font-black mb-2">Nenhum produto encontrado</h2>
                        <p className="text-muted-foreground max-w-md">
                            Tente ajustar os filtros ou remover a categoria selecionada para ver mais produtos.
                        </p>
