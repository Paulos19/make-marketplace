...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!userId) {
      return NextResponse.json({ message: 'Sessão não encontrada ou expirada. Por favor, faça login novamente.' }, { status: 401 });
    }
    ...
    if (!user) {
      return NextResponse.json({ message: 'Utilizador não encontrado.' }, { status: 401 });
    }

    if (user.role !== UserRole.SELLER && user.role !== UserRole.ADMIN) {
      return NextResponse.json({ message: `A sua permissão é '${user.role}'. É necessário ser um VENDEDOR.` }, { status: 403 });
    }
