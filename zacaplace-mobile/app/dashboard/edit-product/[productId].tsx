...
const EditProductScreen = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const productResponse = await apiClient.get<ProductDetail>(`/api/products/${productId}`);
        ...
        setOnPromotion(productData.onPromotion);
        setIsService(productData.isService);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await apiClient.put<any>(`/api/products/${productId}`, productData);
      CustomAlert.alert('Sucesso', 'Produto atualizado com sucesso!');
      router.back();
    } catch (err: any) {
      console.error('Erro ao atualizar produto:', err);
      if (err instanceof ApiError) {
        CustomAlert.alert('Erro', err.data.message || err.message);
      } else {
        CustomAlert.alert('Erro', 'Ocorreu um erro inesperado ao atualizar o produto.');
      }
    } finally {
      setSubmitting(false);
    }
  ...
  if (loading) {
    return (
      <View style={styles.centered}>
        ...
        <Text style={{ marginTop: 10 }}>Carregando dados do produto...</Text>
      </View>
    );
  }
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.contentContainer}>
      ...
      <TouchableOpacity style={styles.submitButton} onPress={handleSubmit} disabled={submitting}>
        {submitting ? (
          ...
          <Text style={styles.submitButtonText}>Atualizar Item</Text>
        )}
      </TouchableOpacity>
    </ScrollView>
  );
